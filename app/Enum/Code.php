...
enum Code: int
{
    ...
    // 系统异常
    const F5000 = 5000; // 系统异常 {flag}：文件不存在
    const F5001 = 5001; // 系统异常 {flag}：获取文件内容失败
    const F5002 = 5002; // 系统异常 {flag}：RSA秘钥配置有误，001 服务器私钥， 002 服务器公钥， 003 用户公钥
    const F5003 = 5003; // 系统异常 {flag}：AES秘钥配置有误, 001 key， 002 iv
    ...
    const F10002 = 10002; // 文件类型{type}与扩展名{ext}不一致
    const F10003 = 10003; // 文件大小{size}超过{maxSize}限制
    const F10004 = 10004; // 不支持的加密方式 {type}
    const F10005 = 10005; // 不支持的签名方式 {type}
    const F10006 = 10006; // 请先校验MFA信息
    ...
    const E100060 = 100060; // 请先取消该用户关联角色[{role}]，否则无权限修改数据！
    const E100061 = 100061; // 您的账号状态当前已被禁用，请联系管理员！
    const E100062 = 100062; // 缺少参数:{param}！
    const E100063 = 100063; // 非有效参数:{param}！
    const E100064 = 100064; // 不存在的ApiKey！
    const E100065 = 100065; // ApiKey未被启用！
    const E100066 = 100066; // 签名错误！
    const E100067 = 100067; // {param}数据无法解密！
    const E100068 = 100068; // 用户尚未绑定MFA设备，无法启用校验功能！
    const E100069 = 100069; // 文件[{file}]记录不存在
    const E100070 = 100070; // 数据类型错误
    const E100071 = 100071; // 敏感数据只有超级管理员有权限编辑哦！
